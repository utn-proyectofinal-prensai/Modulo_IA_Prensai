...
def es_declaracion_ollama(texto, ministro_key_words, ministerios_key_words=None):
    ...
    prompt = (
        ...
        "IMPORTANTE:\n"
        "- Si hay AL MENOS UNA cita textual entre comillas atribuida a un actor, es DECLARACIÓN.\n"
        "- La atribución puede estar fuera de las comillas (antes o después), pero debe referirse claramente a uno de los actores listados.\n"
        f"TEXTO: {texto}\n\n"
        "Respondé únicamente SI o NO:"
    )
...
def _promptear_clasificacion_tema_ollama(texto, lista_temas, tema_default, url_id=None):
    ...
    prompt = (
        ...
        "REGLAS IMPORTANTES:\n"
        "- Elegí un tema específico solo si el texto trata principalmente sobre ese tema o espacio.\n"
        "- Si la mención es tangencial, anecdótica o apenas nombran el lugar mientras se desarrolla otro asunto, usá el tema genérico '{tema_default}'.\n"
        "- Ante la duda, preferí '{tema_default}' en lugar de forzar un tema específico.\n"
        "- NUNCA ignores un tema específico que está claramente mencionado y es el eje principal.\n"
        "- Si hay dudas entre temas similares, elegí el MÁS ESPECÍFICO solo cuando la evidencia sea fuerte.\n\n"
